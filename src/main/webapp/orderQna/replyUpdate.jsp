   ...
   $(document).click(function(e) {
	...
	function validateForm(state) {
		...
		if(state == "답변완료"){
			...
			location.href = "<%=request.getContextPath() %>/orderQna/replyUpdate.oq?pur_q_seq=<%=pur_q_seq %>&state=<%=state%>&pageNum=<%=p.getPageNum()%>";
			return;
	...
	function isReply(pur_q_seq) {
		isReply = 1;
		location.href="replyWrite.oq?pur_q_seq=" + pur_q_seq + "&isReply=<%=isReply %>" + "&state=<%=state%>&pageNum=<%=p.getPageNum()%>";
	}
	...
	<main id="container">
				...
				<tbody>
					...
					<% } else { %>
						<% for(OrderQnaDTO qna : qnaList){ %>	
							...
							<tr class="reply_wrap">
								<td colspan="5">
									<div class="reply_wrap_content reply">
									...
									<div class="reply_input">
										<% if (pur_q_seq == reply.getPur_q_seq() && isReply == 1) { %>
											<ul class="reply_form">
												<li class="">
													<form class="write_form" name="writeForm" method="post" action="/EIBooks/orderQna/replyUpdateProc.oq?<%if(state != 5) {%>state=<%=state%>&<%}%>pageNum=<%=p.getPageNum()%>">
														<div class="text_area">
		...
		<% if(!qnaList.isEmpty()) { %>
			<div class="pagination">
				<%if(p.isPrev()) {%>
				<a class="first arrow" href="replyUpdate.oq?pur_q_seq=<%=pur_q_seq %><%if(state != 5) { %>&state=<%=state %><%} %>&isReply=1&pageNum=1">
				...
				<%if(p.isPrev()) {%>
				<a class="prev arrow" href="replyUpdate.oq?pur_q_seq=<%=pur_q_seq %>&isReply=1<%if(state != 5) { %>&state=<%=state %><%} %>&pageNum=<%=p.getStartPage()-1 %>">
				...
				<%for(int i=p.getStartPage(); i<=p.getEndPage(); i++) {%>
					...
					<%}else {%>
						<a class="number" href="replyUpdate.oq?pur_q_seq=<%=pur_q_seq %>&isReply=1<%if(state != 5) { %>&state=<%=state %><%} %>&pageNum=<%=i %>"><%=i %></a>
					<%} %>
				<%} %>
				
				<%if(p.isNext()) {%>
				<a class="next arrow" href="replyUpdate.oq?pur_q_seq=<%=pur_q_seq %>&isReply=1<%if(state != 5) { %>&state=<%=state %><%} %>&pageNum=<%=p.getEndPage()+1 %>">
				...
				<%if(p.isNext()) {%>
				<a class="last arrow" href="replyUpdate.oq?pur_q_seq=<%=pur_q_seq %>&isReply=1<%if(state != 5) { %>&state=<%=state %><%} %>&pageNum=<%=p.getRealEnd() %>">
