	...
	<main id="container">
		...
		<ul class="sort_wrap">
				...
				<li class="sort_menu">
					<ul>
						...
						<li>
							<a href="/EIBooks/orderQna/qnaList.oq?state=0">답변대기</a>
						</li>
						<li>
							<a href="/EIBooks/orderQna/qnaList.oq?state=1">답변완료</a>
						</li>
					</ul>
				</li>
			</ul>
		...
		<% if(!qnaList.isEmpty()) { %>
			<div class="pagination">
				<%if(p.isPrev()) {%>
				<a class="first arrow" href="qnaList.oq?<%if(state != 5) { %>state=<%=state %>&<%} %>pageNum=1">
				...
				<%if(p.isPrev()) {%>
				<a class="prev arrow" href="qnaList.oq?<%if(state != 5) { %>state=<%=state %>&<%} %>pageNum=<%=p.getStartPage()-1 %>">
				...
				<%for(int i=p.getStartPage(); i<=p.getEndPage(); i++) {%>
					...
					<%}else {%>
						<a class="number" href="qnaList.oq?<%if(state != 5) { %>state=<%=state %>&<%} %>pageNum=<%=i %>"><%=i %></a>
					<%} %>
				<%} %>
				
				<%if(p.isNext()) {%>
				<a class="next arrow" href="qnaList.oq?<%if(state != 5) { %>state=<%=state %>&<%} %>pageNum=<%=p.getEndPage()+1 %>">
				...
				<%if(p.isNext()) {%>
				<a class="last arrow" href="qnaList.oq?<%if(state != 5) { %>state=<%=state %>&<%} %>pageNum=<%=p.getRealEnd() %>">
