...
<tr>
<td colspan="6">
<%if(p.isPrev()) {%><a href="reviewWrite.do?bookNum=<%=bookNum %>&userNum=<%=userNum %>&pageNum=<%=p.getStartPage() %>&orderBy=<%=orderBy %>">[처음]</a><% } %>
...
<%if(p.isNext()) {%><a href="reviewWrite.do?bookNum=<%=bookNum %>&userNum=<%=userNum %>&pageNum=<%=p.getEndPage() %>&orderBy=<%=orderBy %>">[마지막]</a><% } %>
</td>
</tr>
<%} %>
</table>
